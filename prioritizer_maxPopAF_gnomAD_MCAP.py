...
for vcfline in vcf.readlines():
	if vcfline.startswith("#"):
		...
		dups=re.search('segmentDups_UCSC', found_ann)
		if dups:
			warn = 1
...
#			found_db_solv = "."
			...
			if "," in found_mcap:
				split_found_mcap = re.split(",", found_mcap)
				found_mcap = split_found_mcap[0]
				warn = 1
                else:
                        found_mcap = "."
		...
		if found_cadd == "." :
			if found_mcap == ".":
				Var_effect_score = 40
			elif float(found_mcap) > 0.025:
				Var_effect_score = 100
			else:
				Var_effect_score = 10
		elif float(found_cadd) > 20: 
			if found_mcap == ".":
				Var_effect_score = 100
			elif float(found_mcap) > 0.025:
				Var_effect_score = 140
			else:
				Var_effect_score = 40
		elif float(found_cadd) <= 20:
			if found_mcap == ".":
...
#		print spline[0], spline[1], "gnomad: ",found_gnomad,"\tdb: ",str(float(subfound_db[0])/float(subfound_db[1])),"\timpact: ",found_impact,"\tcadd: ",found_cadd,"\tmcap: ",found_mcap,"\trvis: ",found_rvis,"\tgdi: ",found_gdi,"\tphenolyzer score: ",found_phlyz
...
#		print spline[0]+"\t"+spline[1]+"\tFinal_score:\t"+str(Final_score)
		if not "." in all_gnomAD_exomes:
			...
			if warn == 1:
				out.write("\tmultiple alleles or low-confidence region\n")
			else:
				out.write("\t.\n")
		else:
